...
import ViolationTypeFormDialog from './ViolationTypeFormDialog.jsx'
import {
  Dialog,
  DialogTrigger,
  DialogPortal,
...
export default function ViolationTypes() {
  ...
  useEffect(() => {
    ...
    violationTypeService
      .listViolationTypes({
        search,
        status: status === 'ALL STATUS' ? undefined : status.toLowerCase(),
        page,
        per_page: 10,
        sort_dir: sortDir,
      })
  ...
  function handleSaved(name, isEdit) {
    ...
    toast({
      variant: 'success',
      title: isEdit ? 'Violation type updated' : 'Violation type created',
      description: `${name} was ${isEdit ? 'updated' : 'created'} successfully.`,
    })
  }

  function handleDeleteRequest(type) {
    setDeleting(type)
    setDeleteOpen(true)
  }
  ...
  async function handleDeleteConfirm() {
    ...
    setDeleteBusy(true)
    try {
      await violationTypeService.deleteViolationType(deleting.id)
      toast({
        variant: 'success',
    ...
    <div className="min-h-full dot-grid">
        ...
        <div className="flex items-center gap-4">
          <div>
            <div className="text-[11px] font-mono uppercase tracking-widest">
              ...
              <span className="text-brand-green">Violation Types</span>
            </div>
            <h1 className="text-2xl font-bold mt-1 text-foreground">Violation Types</h1>
          </div>
        </div>
              ...
              {loading ? (
                ...
                types.map((type) => (
                  <TableRow key={type.id}>
                    ...
                    <TableCell className="px-5 py-3.5 text-muted-foreground max-w-[24rem]">
                      <span className="line-clamp-2">{type.description || '—'}</span>
                    </TableCell>
                    <TableCell className="px-5 py-3.5">
                      <StatusChip isActive={type.is_active} />
                    </TableCell>
                    <TableCell className="px-5 py-3.5 font-mono text-xs text-muted-foreground">
      ...
      <Dialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        ...
        <DialogPortal>
          <DialogBackdrop />
          <DialogPopup>
            <DialogTitle className="text-base font-semibold text-foreground">Delete violation type?</DialogTitle>
            <DialogDescription className="mt-1">
              This will permanently remove {deleting?.name || 'this violation type'}. This action cannot be
              undone.
            </DialogDescription>
