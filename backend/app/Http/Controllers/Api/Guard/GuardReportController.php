...
class GuardReportController extends Controller
{
    ...
    public function academicYears(Request $request): JsonResponse
    {
        $years = AcademicYear::query()
            ->when($request->filled('search'), fn ($q) => $q->where(function ($q) use ($request): void {
                $q->where('code', 'like', '%'.trim($request->input('search')).'%')
    ...
    private function filters(Request $request): array
    {
        ...
        $labels[] = 'Recorded By: '.$request->user()->name;
    ...
    private function violations(Request $request): array
    {
        ...
        return $this->payload('Violations Report', 'guard-violations-report', $f, [
            'kpis' => [
    ...
    private function attendance(Request $request): array
    {
        ...
        return $this->payload('Attendance Report', 'guard-attendance-report', $f, [
            'kpis' => [
    ...
    private function visitors(Request $request): array
    {
        ...
        $query = VisitorRegistration::query()
            ->with('timeLogs')
            ->whereHas('timeLogs', fn ($q) => $q->where('performed_by', $guardId))
        ...
        return $this->payload('Visitor Log Report', 'guard-visitor-log-report', $f, [
            'kpis' => [
    ...
    private function compliance(Request $request): array
    {
        ...
        $query = Compliance::query()
            ->with('room')
            ->where('recorded_by', $request->user()->name)
            ->whereBetween('created_at', [$f['from'], $f['to']]);
        ...
        return $this->payload('Compliance Report', 'guard-compliance-report', $f, [
            'kpis' => [
    ...
    private function summary(Request $request): array
    {
        ...
        $violations = StudentViolation::where('recorded_by', $guardId)->whereBetween('created_at', [$f['from'], $f['to']]);
        $visitors = VisitorRegistration::whereHas('timeLogs', fn ($q) => $q->where('performed_by', $guardId))
            ->whereBetween('created_at', [$f['from'], $f['to']]);
        ...
        $resolved = StudentViolation::where('recorded_by', $guardId)
            ->whereBetween('created_at', [$f['from'], $f['to']])
        ...
        $checkIns = StudentTimeLog::where('performed_by', $guardId)
            ->whereBetween('time', [$f['from'], $f['to']])
            ->when($f['academic_year_id'], fn ($q) => $q->where('academic_year_id', $f['academic_year_id']))
            ->where('type', 'in')
            ->count();
        ...
        $typeNames = ViolationType::pluck('name', 'id');
        $topTypes = StudentViolation::where('recorded_by', $guardId)
            ->whereBetween('created_at', [$f['from'], $f['to']])
            ->when($f['academic_year_id'], fn ($q) => $q->whereHas('student', fn ($sq) => $sq->where('academic_year_id', $f['academic_year_id'])))
        ...
        $topOffenders = StudentViolation::where('recorded_by', $guardId)
            ->whereBetween('created_at', [$f['from'], $f['to']])
            ->when($f['academic_year_id'], fn ($q) => $q->whereHas('student', fn ($sq) => $sq->where('academic_year_id', $f['academic_year_id'])))
            ->with('student')
            ->get()
        ...
        $daily = $this->dailySeries($f, $guardId);

        return $this->payload('Executive Summary Report', 'guard-executive-summary-report', $f, [
            'kpis' => [
    ...
    private function dailySeries(array $f, int $guardId): Collection
    {
